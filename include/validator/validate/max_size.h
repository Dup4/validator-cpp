#ifndef VALIDATOR_VALIDATE_MAX_SIZE_H
#define VALIDATOR_VALIDATE_MAX_SIZE_H
...
public:
    ...
    MaxSize(size_t size, Opt&&... opts) : size_(size) {
    ...
    Result Validate(const T& t) {
        ...
        std::string error_message_pattern =
                "`$field_name` is to big. Max size is $max_size, but actual is $actual_size.";
        std::map<std::string, std::string> replace_map = {
                {"$field_name", options_.field_name.value_or("")},
                {"$max_size", std::to_string(size_)},
...
#endif  // VALIDATOR_VALIDATE_MAX_SIZE_H
